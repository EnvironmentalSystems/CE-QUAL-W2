...
IF(LAKE_RIVER_CONTOUR_ON=='ON')THEN    ! SW 2/28/2020
          DO JJ=1,NUM_LAKE_CONTOUR
              IF(JDAY >= NXT_LAKE_CONTOUR(JJ))THEN
                  ...
                  WRITE(LAKE_RIVER_CONTOUR+JJ-1,'(F10.4,",",1000(F8.2,","))')JDAY,ELWS(LAKE_CONTOUR_SEG(JJ))+0.1,TBLANK
                   WRITE(LAKE_RIVER_CONTOUR+JJ-1,'(F10.4,",",1000(F8.2,","))')JDAY,ELWS(LAKE_CONTOUR_SEG(JJ)),T2(KTWB(JW_LAKE_CONTOUR(JJ)),LAKE_CONTOUR_SEG(JJ))
                  DO K=KTWB(JW_LAKE_CONTOUR(JJ))+1,KB(LAKE_CONTOUR_SEG(JJ))
                      WRITE(LAKE_RIVER_CONTOUR+JJ-1,'(F10.4,",",1000(F8.2,","))')JDAY,(EL(K,LAKE_CONTOUR_SEG(JJ))+EL(K,LAKE_CONTOUR_SEG(JJ)))*0.5,T2(K,LAKE_CONTOUR_SEG(JJ))
                  ENDDO
                  ELSE
                  WRITE(LAKE_RIVER_CONTOUR+JJ-1,'(F10.4,",",1000(F8.2,","))')JDAY,(TBLANK,K=2,KTWB(JW_LAKE_CONTOUR(JJ))-1),(T2(K,LAKE_CONTOUR_SEG(JJ)),K=KTWB(JW_LAKE_CONTOUR(JJ)),KB(LAKE_CONTOUR_SEG(JJ)))
                  ENDIF
                  IF(OXYGEN_DEMAND)THEN
                    IF(LAKE_CONTOUR_FORMAT == 1)THEN
                    WRITE(LAKE_RIVER_CONTOUR+10+JJ-1,'(F10.4,",",1000(F8.2,","))')JDAY,ELWS(LAKE_CONTOUR_SEG(JJ))+0.1,TBLANK
                    WRITE(LAKE_RIVER_CONTOUR+10+JJ-1,'(F10.4,",",1000(F8.2,","))')JDAY,ELWS(LAKE_CONTOUR_SEG(JJ)),C2(KTWB(JW_LAKE_CONTOUR(JJ)),LAKE_CONTOUR_SEG(JJ),NDO)
                    DO K=KTWB(JW_LAKE_CONTOUR(JJ))+1,KB(LAKE_CONTOUR_SEG(JJ))
                      WRITE(LAKE_RIVER_CONTOUR+10+JJ-1,'(F10.4,",",1000(F8.2,","))')JDAY,(EL(K,LAKE_CONTOUR_SEG(JJ))+EL(K,LAKE_CONTOUR_SEG(JJ)))*0.5,C2(K,LAKE_CONTOUR_SEG(JJ),NDO)
                    ENDDO
                    ELSE
                    WRITE(LAKE_RIVER_CONTOUR+10+JJ-1,'(F10.4,",",1000(F8.2,","))')JDAY,(TBLANK,K=2,KTWB(JW_LAKE_CONTOUR(JJ))-1),(C2(K,LAKE_CONTOUR_SEG(JJ),NDO),K=KTWB(JW_LAKE_CONTOUR(JJ)),KB(LAKE_CONTOUR_SEG(JJ)))
                    ENDIF                 
                  ENDIF
              ENDIF    
          ENDDO
          DO JJ=1,NUM_RIVER_CONTOUR
              IF(JDAY >= NXT_RIVER_CONTOUR(JJ))THEN
                  ...
                  DO I=US(JB),DS(JB)
                      WRITE(LAKE_RIVER_CONTOUR+20+JJ-1,'(F10.4,",",1000(F8.2,","))')JDAY,X1(I),T2(KTWB(JW_RIVER_CONTOUR(JJ)),I)
                  ENDDO
                  ENDDO
                  ELSE
                  WRITE(LAKE_RIVER_CONTOUR+20+JJ-1,'(F10.4,",",1000(F8.2,","))')JDAY,((T2(KTWB(JW_RIVER_CONTOUR(JJ)),I),I=US(JB),DS(JB)),JB=RIVER_CONTOUR_BR1(JJ),RIVER_CONTOUR_BR2(JJ))
                  ENDIF
                    IF(OXYGEN_DEMAND)THEN
                    IF(RIVER_CONTOUR_FORMAT == 1)THEN
                    DO JB=RIVER_CONTOUR_BR1(JJ),RIVER_CONTOUR_BR2(JJ)
                    DO I=US(JB),DS(JB)
                      WRITE(LAKE_RIVER_CONTOUR+30+JJ-1,'(F10.4,",",1000(F8.2,","))')JDAY,X1(I),C2(KTWB(JW_RIVER_CONTOUR(JJ)),I,NDO)
                    ENDDO
                    ENDDO
                    ELSE
                    WRITE(LAKE_RIVER_CONTOUR+30+JJ-1,'(F10.4,",",1000(F8.2,","))')JDAY,((C2(KTWB(JW_RIVER_CONTOUR(JJ)),I,NDO),I=US(JB),DS(JB)),JB=RIVER_CONTOUR_BR1(JJ),RIVER_CONTOUR_BR2(JJ))
                    ENDIF             
                    ENDIF
  ...
  IF (TIME_SERIES) THEN  
    ...
    IF(K >= KTWB(JW))THEN      ! SW 4/4/2018 ADDED TO ELIMINATE THE LAST VALUE OF A VARIABLE BEING USED FOR CASE WHEN ESTR IS NEGATIVE
        ...
        IF (ICE_COMPUTATION) THEN  
          IF(SEDIMENT_CALC(JW))THEN  
            WRITE (TSR(J),'(f10.3,",",19(F10.3,","),1000(A,","))') JDAY,DLT,ELWS(I),T1(K,I),U(K,I),QC(I),SRON(JW)*1.06,GAMMA(K,I),DEPTHB(KB(I),I),    &     ! SW 8/13/06  
     ...
     ELSE      ! SW 4/4/2018
        IF (ICE_COMPUTATION) THEN  
          IF(SEDIMENT_CALC(JW))THEN  
            WRITE (TSR(J),'(f10.3,",",19(F10.3,","),1000(F10.1,","))') JDAY,DLT,ELWS(I),-99.,-99.,QC(I),SRON(JW)*1.06,-99.,DEPTHB(KB(I),I),    &     ! SW 8/13/06  
        ...
        ELSE  
          IF(SEDIMENT_CALC(JW))THEN  
            WRITE (TSR(J),'(f10.3,",",18(F10.3,","),1000(F10.1,","))') JDAY,DLT,ELWS(I),-99.,-99.,QC(I),SRON(JW)*1.06,-99.,DEPTHB(KB(I),I),    &     ! SW 8/13/06  
...
3312            FORMAT(5X,'Macrophyte spec ',i2,/7X,'Spatially integrated mass [MACMBRS] = ',1PE15.8E2,1X,'g ',/7X,   &  
    ...
    IF (PROFILE(JW)) THEN  
      IF (JDAY >= NXTMPR(JW) .OR. JDAY >= PRFD(PRFDP(JW)+1,JW)) THEN  
        ...
        elseif(jw == 1)then   ! write out individual files on these days
             ...
             IF(CONSTITUENTS)THEN
             WRITE(NUNIT,'(1000(A,","))')'Seg#','ElevWaterSurf(m)','Q(m3/s)','SurfaceTemp(oC)','Depth(m)','Width(m)','VolWeighTemp(oC)' ,(CNAME2(CN(JC)),JC=1,NAC),(CDNAME2(CDN(JD,JW)),JD=1,NACD(JW))
             ELSE
                 ...
                 DO JJ=1,NWB
                     ...
                     DO JB=BS(JJ),BE(JJ)
                         DO I=CUS(JB),DS(JB)
                             ...
                             IF(CONSTITUENTS)THEN
                             WRITE(NUNIT,'(I5,",",100(F12.4,","))')I,ELWS(I),QC(I),T2(K,I),DEPTHB(KB(I),I),B(KTI(I),I),TVOLAVG,(C2(K,I,CN(JAC))*CMULT(CN(JAC)),JAC=1,NAC),(CD(K,I,CDN(JD,JW))*CDMULT(CDN(JD,JW)),JD=1,NACD(JW))
                             ELSE
    ...
    IF (SPREADSHEET(JW)) THEN  
      IF (JDAY >= NXTMSP(JW) .OR. JDAY >= SPRD(SPRDP(JW)+1,JW)) THEN  
        ...
        ENDDO          
        WRITE (SPRV(JW),'(A,",",F10.3,",",1000(A,","))') 'Temperature',JDAY,(CONV1(K,ISPR(J,JW)),J=1,NISPR(JW))  
        ...
        DO JC=1,NAC  
          IF (PRINT_CONST(CN(JC),JW)) THEN  
            ...
            END DO   
              WRITE (SPRV(JW),'(A38,",",F10.3,",",1000(A,","))') CNAME3(CN(JC)),JDAY,(CONV1(K,ISPR(J,JW)),J=1,NISPR(JW))   
          END IF  
        END DO  
        IF (CONSTITUENTS) THEN  
          DO JD=1,NACD(JW)  
            IF (PRINT_DERIVED(CDN(JD,JW),JW)) THEN  
              DO J=1,NISPR(JW)  
                  ...
                  WRITE (CONV1(K,ISPR(J,JW)),'(F12.4)') TVOLAVG                                     ! SW 8/31/18  
              END DO  
                WRITE (SPRV(JW),'(A38,",",F10.3,",",1000(A,","))') CDNAME3(CDN(JD,JW)),JDAY,(CONV1(K,ISPR(J,JW)),J=1,NISPR(JW))  
            END IF  
            END DO  
            END IF    
...
9999      FORMAT (200(e13.6,1x))  
  ...
  IF (DOWNSTREAM_OUTFLOW) THEN  
   ...
   !     JWD=0                        ! sw 5/17/13
        ...
        END DO  
2499      WRITE (WDO(J,1),'(F10.3,",",F9.3,",",8X,100(F9.3,","))',ERR=2500) JDAY, QWDO(J), (QOUTLET(I),I=1,NUMOUTLETS)     ! sw 3/2019 This code was necessary during multiple WB read/write possible error with one file copying and another writing at the same time
          GO TO 2501
2500      CALL SLEEPQQ(100)          
          ...
          go to 2499
2501      CONTINUE
          WRITE (WDO(J,2),'(F10.3,",",F8.2,",",8X,100(F8.2,","))',ERR=2502) JDAY, TWDO(J), (TOUTLET(I),I=1,NUMOUTLETS)  
          GO TO 2503
2502      CALL SLEEPQQ(100)          
          ...
          go to 2501
2503      CONTINUE
          IF (CONSTITUENTS) WRITE (WDO(J,3),'(F10.3,",",1000(A10,","))',ERR=2504) JDAY,(CWDOC(CN(JC)),     JC=1,NAC)  
          GO TO 2505

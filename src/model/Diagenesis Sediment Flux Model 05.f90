...
Module CEMASedimentDiagenesis   
  ...
  Real(R8) SD_JMn2, SD_JT, SD_Ksw, SD_rhowcp, SD_tsed
  ...
  Subroutine SedimentFlux
    ...
    DO SegNumI = IU, ID
        ...
        DO LayerNum = KT, KB(SegNumI)
          ...
          SD_CH40     =   MAX(C2(LayerNum,SegNumI,NCH4),0.0)
          SD_H2S0     =   MAX(C2(LayerNum,SegNumI,NH2S),0.0)
  ...
  Subroutine SedimentPOM
    implicit none
    !
    real(R8) :: FAP3, FEP3, FCBOD3, AlgaeSed, EPBurial
    integer  :: JJJ, ig
    !
    ...
    DO JJJ=1,NBOD
      ...
      IF(BOD_CALCN(JJJ)) THEN
          SDINN1(LayerNum,SegNumi) = SDINN1(LayerNum,SegNumi) + MAX(CBODS(JJJ)*DAY,0.0)*RBOD(JJJ)*CBODN(LayerNum,SegNumi,J)
      ELSE    
  ...
  Subroutine ComputeSOD
    ...
    SD_CH4SAT = 100.0D+00*(1.0D+00 + SD_depth/10.0D+00)*(1.024**(20.0D+00 - SD_Tw))
    ...
    ! check if SOD solution is converged
		If (it >= maxit) Write(WRN,*) 'Sediment Diangenesis: SOD iterations exceeded'
  End Subroutine
...
!					
    ...
    Dissolved_SO4_Src   = Dissolved_SO4_Src/DAY      !g/m³/d --> g/m³/s
		...
		Dissolved_H2S_Src   = Dissolved_H2S_Src/DAY      !g/m³/d --> g/m³/s
...
!**** pH, carbon dioxide, bicarbonate, and carbonate concentrations
  ...
  Subroutine Lin_Sys(a11, a12, a21, a22, b1, b2, x1, x2)   !,NFLog,NFCle) 
	  ...
	  If (a11 * a22 - a12 * a21 == 0) Then 
		  ...
		  Write(w2err,'(a)') 'The sediment flux solution matrix is singular: '

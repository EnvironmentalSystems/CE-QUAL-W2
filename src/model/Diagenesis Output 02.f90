...
!            Write(CEMATSR1OutFilN,'(f14.6 , "," ,i5,",",i5, ",", f14.6, ",", f14.6, ",", f14.6, ",", f14.6, ",", f14.6, ",", f14.6, ",", f14.6, ",", f14.6)')JDAY, KTWB(1), KB(4), z(4), BedElevation(4), BedElevationLayer(4), PorewaterRelRate(4)*86400.d0, BedPorosity(4)*100, BedConsolidRate(4)*86400.d0, EL(KTWB(1),4)-Z(4), CEMACumPWRelease(4)
    ...
    Subroutine WriteCEMASedimentFluxOutput
        ...
        If(WriteCEMAMFTSedFlx)Then
            ...
            Write(CEMASedFlxFilN4,'("SOD(gO2/m2/d),",f12.4,",",<IMX>(f10.4,","),<IMX>(f10.4,","))')JDAY,(KFSF(KB(SegNumI),SegNumI,4),  SegNumI = 1, IMX),(KFSFAV(SegNumI,1), SegNumI = 1, IMX)
            Write(CEMASedFlxFilN5,'("POCG1(g/m3),",f12.4,",",<IMX>(f10.4,","),<IMX>(f10.4,","))')  JDAY,(C2SF(KB(SegNumI),SegNumI,19), SegNumI = 1, IMX),(KFSFAV(SegNumI,2), SegNumI = 1, IMX)
            Write(CEMASedFlxFilN6,'("POCG2(g/m3),",f12.4,",",<IMX>(f10.4,","),<IMX>(f10.4,","))')  JDAY,(C2SF(KB(SegNumI),SegNumI,20), SegNumI = 1, IMX),(KFSFAV(SegNumI,3), SegNumI = 1, IMX)
            Write(CEMASedFlxFilN7,'("JC(gC/m2/d),",f12.4,",",<IMX>(f10.4,","),<IMX>(f10.4,","))')           JDAY,(KFSF(KB(SegNumI),SegNumI,7),  SegNumI = 1, IMX),(KFSFAV(SegNumI,4), SegNumI = 1, IMX)
            Write(CEMASedFlxFilN8,'("JN(gN/m2/d),",f12.4,",",<IMX>(f10.4,","),<IMX>(f10.4,","))')           JDAY,(KFSF(KB(SegNumI),SegNumI,8),  SegNumI = 1, IMX),(KFSFAV(SegNumI,5), SegNumI = 1, IMX)
            Write(CEMASedFlxFilN9,'("PONG1(g/m3),",f12.4,",",<IMX>(f10.4,","),<IMX>(f10.4,","))')  JDAY,(C2SF(KB(SegNumI),SegNumI,22), SegNumI = 1, IMX),(KFSFAV(SegNumI,6), SegNumI = 1, IMX)
            Write(CEMASedFlxFilN10,'("PONG2(g/m3),",f12.4,",",<IMX>(f10.4,","),<IMX>(f10.4,","))') JDAY,(C2SF(KB(SegNumI),SegNumI,23), SegNumI = 1, IMX),(KFSFAV(SegNumI,7), SegNumI = 1, IMX)
            
            Write(CEMASedFlxFilN11,'("JCH4(gO2/m2/d),",f12.4,",",<IMX>(f10.4,","),<IMX>(f10.4,","))')JDAY,(KFSF(KB(SegNumI),SegNumI,10), SegNumI = 1, IMX),(KFSFAV(SegNumI,8),  SegNumI = 1, IMX)
            Write(CEMASedFlxFilN12,'("JNH4(gN/m2/d),",f12.4,",",<IMX>(f10.4,","),<IMX>(f10.4,","))')JDAY,(KFSF(KB(SegNumI),SegNumI,14), SegNumI = 1, IMX),(KFSFAV(SegNumI,9),  SegNumI = 1, IMX)
            Write(CEMASedFlxFilN13,'("JNO3(gN/m2/d),",f12.4,",",<IMX>(f10.4,","),<IMX>(f10.4,","))')JDAY,(KFSF(KB(SegNumI),SegNumI,15), SegNumI = 1, IMX),(KFSFAV(SegNumI,10), SegNumI = 1, IMX)
            Write(CEMASedFlxFilN14,'("JPO4(gP/m2/d), ,",f12.4,",",<IMX>(f10.4,","),<IMX>(f10.4,","))')JDAY,(KFSF(KB(SegNumI),SegNumI,16), SegNumI = 1, IMX),(KFSFAV(SegNumI,11), SegNumI = 1, IMX)
            
            Write(CEMASedFlxFilN15,'("POPG1(g/m3),",f12.4,",",<IMX>(f10.4,","),<IMX>(f10.4,","))')JDAY,(C2SF(KB(SegNumI),SegNumI,25), SegNumI = 1, IMX),(KFSFAV(SegNumI,12), SegNumI = 1, IMX)
            Write(CEMASedFlxFilN16,'("POPG2(g/m3),",f12.4,",",<IMX>(f10.4,","),<IMX>(f10.4,","))')JDAY,(C2SF(KB(SegNumI),SegNumI,26), SegNumI = 1, IMX),(KFSFAV(SegNumI,13), SegNumI = 1, IMX)
            
            Write(CEMASedFlxFilN17,'("CSOD(gO2/m2/d),",f12.4,",",<IMX>(f10.4,","),<IMX>(f10.4,","))')JDAY,(KFSF(KB(SegNumI),SegNumI,5), SegNumI = 1, IMX),(KFSFAV(SegNumI,14), SegNumI = 1, IMX)
            Write(CEMASedFlxFilN18,'("NSOD(gO2/m2/d),",f12.4,",",<IMX>(f10.4,","),<IMX>(f10.4,","))')JDAY,(KFSF(KB(SegNumI),SegNumI,6), SegNumI = 1, IMX),(KFSFAV(SegNumI,15), SegNumI = 1, IMX)
            Write(CEMASedFlxFilN19,'("JP(gP/m2/d),",f12.4,",",<IMX>(f10.4,","),<IMX>(f10.4,","))')            JDAY,(KFSF(KB(SegNumI),SegNumI,9), SegNumI = 1, IMX),(KFSFAV(SegNumI,16), SegNumI = 1, IMX)

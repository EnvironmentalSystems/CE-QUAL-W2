...
! 
  ...
  INTEGER     :: it, n, ig
  targetfnno = 8888
  open (targetfnno+1,file='w2_TDGtarget.npt',status='old')
  ...
  IF (tsdynsel=='      ON') THEN
     open (targetfnno+3,file='TDGdyntarget.npt',status='old')          
     read (targetfnno+3,'(///2F8.3)') nxtjday, tstarget2         
     tstarget = tstarget2                                         
     read (targetfnno+3,'(2F8.3)') nxtjday, tstarget2              
  END IF
!!  Initial output file  
  open  (targetfnno, FILE='TDGTarget_output.opt'  , status='unknown')
  ...
  IF (Q_SP>0.0) THEN     
      ...
      IF (DAYTEST >= tstsrt .AND. DAYTEST<=tstend .AND. TDG_TDG>tstarget+tsconv) THEN
         ! INITIAL VARIABLES FOR ITERATIONS
          ...
          DO ig = 1, NGSP
             Q_SP = Q_SP + QGT(SPGTNO(ig))
             IF (QGT(SPGTNO(ig))>0.0 .AND. QGT(SPGTNO(ig))>Q_ALL*SPMINFRAC(SPGTNO(ig))+0.0000000001) SP_ACTIVE(ig)=.TRUE.
          END DO
          DO ig = 1, NGPH
             Q_PH = Q_PH + QGT(PHGTNO(ig))
             IF (QGT(PHGTNO(ig))>0.0.AND. QGT(SPGTNO(ig))+0.0000000001<PHMAXFLOW(ig)) PH_ACTIVE(ig)=.TRUE.
          END DO
         ...
         DO WHILE (ABS(TDG_TDG-tstarget)>tsconv .AND. ITERATION<=tsiteration .AND. Q_CUT>0.0)
            ...
            END DO
            Q_TEMP = 0.0
            DO WHILE (Q_CUTTED+0.0000000001<Q_CUT .AND. Q_LEFT>Q_ALL*SUM_SP_FRAC+0.0000000001)  ! CUT FLOW TO SP UNTILL Q_CUTTED = Q_CUT
                ! UPDATE PRIOR TOP
                ...
                END IF
                DO ig = 1, priortop_n
                   IF (QGT(SPGTNO(priortop_spno(ig)))>Q_ALL*SPMINFRAC(priortop_spno(ig))+0.0000000001)  SP_ACTIVE(priortop_spno(ig))=.TRUE.
                END DO
         ...
         IF (ITERATION==1 .AND. Q_SP==0.0) WRITE (targetfnno+2, *) 'THE SPILL FLOW IS ZERO ON JDAY', JDAY
         IF (ITERATION==1 .AND. TDG_TDG < tstarget+tsconv) WRITE (targetfnno+2, *) 'THE tdg IS LOWER THAN TARGET, NO ITERATION CALCULATION NEEDED ON JDAY', JDAY
         IF (TDG_TDG-tstarget>tsconv .AND. ITERATION>tsiteration .AND. Q_SP>0.0)THEN
            WRITE (targetfnno+2, *) 'THE ITERATION TIMES IS OUT OF LIMIT, AND THE TDG IS STILL NOT CONVERGENT TO TARGET ON JDAY', JDAY
            WARNING_OPEN = .TRUE.
